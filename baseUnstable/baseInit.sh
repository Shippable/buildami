...
docker_install() {
  ...
  install_ca_certificates='sudo apt-get install -y apt-transport-https ca-certificates'
  exec_cmd "$install_ca_certificates"

  install_kernel_extras='sudo apt-get install -y linux-image-extra-`uname -r`'
  exec_cmd "$install_kernel_extras"

  add_docker_repo_keys='sudo apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 58118E89F3A912897C070ADBF76221572C52609D'
  exec_cmd "$add_docker_repo_keys"

  add_docker_repo='echo "deb https://apt.dockerproject.org/repo ubuntu-trusty main" | sudo tee -a /etc/apt/sources.list.d/docker.list'
  exec_cmd "$add_docker_repo"

  _run_update

  install_docker='sudo apt-get install -y docker-engine=$DOCKER_VERSION'
  exec_cmd "$install_docker"

  get_static_docker_binary='wget https://get.docker.com/builds/Linux/x86_64/docker-1.11.1.tgz -P /tmp/docker'
  exec_cmd "$get_static_docker_binary"

  extract_static_docker_binary='sudo tar -xzf /tmp/docker/docker-1.11.1.tgz --directory /opt'
